...
export const generateDREReport = (data: ReportData) => {
  ...
  // Receitas (Entradas)
  if (data.entryCategories.length > 0) {
    checkPageBreak(40);
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
  ...
  // Despesas (Saídas)
  if (data.exitCategories.length > 0) {
    checkPageBreak(40);
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
  ...
  Object.entries(transactionsByCategory).forEach(([categoryName, transactions]) => {
    ...
    transactions.forEach((transaction) => {
      ...
      yPosition += 6;
      doc.text(`Descrição: ${transaction.description.substring(0, 60)}${transaction.description.length > 60 ? '...' : ''}`, margin + 10, yPosition);
      yPosition += 6;
  ...
  // Salvar o PDF
  const fileName = `DRE_${data.selectedAccount}_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(fileName);
};
